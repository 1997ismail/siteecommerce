...
if(isset($_POST['delete'])){
   ...
   $delete_wishlist_item = $conn->prepare("DELETE FROM `wishlist` WHERE id ='$wishlist_id'");
   $delete_wishlist_item->execute([$wishlist_id]);
}

if(isset($_GET['delete_all'])){
   $delete_wishlist_item = $conn->prepare("DELETE FROM `wishlist` WHERE id ='$id'");
   $delete_wishlist_item->execute([$id]);
...
<section class="products">
   ...
   <?php
      $grand_total = 0;
      $select_wishlist = $conn->prepare("SELECT * FROM `wishlist` WHERE id ='$id'");
